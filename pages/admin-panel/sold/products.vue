    ...
    <table cellpadding="0" cellspacing="0" class="store__table-product">
      <tbody>
        ...
        <template v-for="item in products">
          <tr :key="item._id">
            <td :colspan="getRoleKey === 'admin' ? 16 : 13">{{ item._id }}</td>
          </tr>
          <tr v-for="product in item.products" :key="product._id + item._id">
              ...
              {{
                Math.round(
                  Number(product.priseSold) -
                    (Number(product.priseSold) / 100) * product.discount -
